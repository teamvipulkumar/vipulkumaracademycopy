...
**Staff Welcome Email**: When `POST /api/staff` succeeds, the server asynchronously dispatches a branded welcome email rendered from the CRM email template of type `staff_welcome` (seeded by `POST /api/admin/crm/templates/seed-defaults`). Variables include `{{name}}`, `{{email}}`, `{{role_name}}`, `{{password}}` (the temp password for new users, or "(unchanged…)" for existing users being elevated), `{{permissions_summary}}` (comma-separated labels), `{{login_url}}`, `{{site_url}}`. Substitution HTML-escapes admin-supplied values to prevent injection. The send is fire-and-forget — request never fails on email errors — and a runtime DDL migration in `runMigrations()` reconciles the legacy CHECK constraints on `email_templates.type` / `email_automation_rules.event` so the new enum value is accepted before the next drizzle-kit push.
...
**Visual Automation Funnel Builder**: Allows creation of multi-step automation funnels with various triggers (e.g., `user_signup`, `new_purchase`) and actions (e.g., `send_email`, `apply_tag`). Supports draft/published states.
